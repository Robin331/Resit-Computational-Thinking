...
def remove_punct(text):
    ...
    'goSouTh'
    """
    exclude = set(string.punctuation) #creates a set named exclude containing all punctuation
    text = ''.join(ch for ch in text if ch not in exclude) #recreates a string containing all the elements of the argument text that are not in the set exclude
    return text
...
def remove_spaces(text):
    ...
    """
    if len(text) > 0:
        while text[len(text) - 1] == " ": #loops through the end of the string removing spaces until it finds something else
            text = text[:-1]
            if len(text) == 0:
                return text
        while text[0] == " ": #loops thourhg the begining of the string removing spaces until it finds something else
            text = text[1:]
            if len(text) == 0:
                return text
    return text
...
def print_menu(exits):
    ...
    # COMPLETE THIS PART:
    for exit in exits: # Iterate over available exits
...
def menu(exits):
    ...
    while True:
        ...
        if is_valid_exit(exits, user_input) == True: # Check if the input makes sense (is valid exit)
            return user_input
